 ...
 */
public class OldExpandLayoutManager extends RecyclerView.LayoutManager {

    private static final int TRANSITION_DURATION = 4000;// todo make it configurable
    ...
    private IAnimationView mIAnimationView;
    ...
    public OldExpandLayoutManager(final int itemSize) {
        mItemSize = itemSize;
        addOpenListener();
        addCloseListener();
...
//            mItemSize = getDecoratedMeasuredHeight(view);
    ...
    public void actionItem(final int pos) {
        if (mAnimatorOpenSet.isRunning() || mAnimatorCloseSet.isRunning()) {
            return;
        }
        if (isOpen()) {
            closeItem(pos);
    ...
    private void openView(final View viewToAnimate) {
        ...
        mIAnimationView = (IAnimationView) viewToAnimate;
    ...
    public void closeItem(final int pos) {// todo refactor because after partial fill, you will need to scroll to item to find it, or force attach it
        ...
        if (null != viewToClose) {
            closeView((IAnimationView) viewToClose);
        }
    }

    private void closeView(final IAnimationView viewToAnimate) {
        mIAnimationView = viewToAnimate;
    ...
    private void addOpenListener() {
        ...
        mAnimatorOpen.addListener(new AnimatorListenerAdapter() {
            @Override
            public void onAnimationStart(final Animator animation) {
                if (null == mIAnimationView) {
                    return;
                }
                mIAnimationView.openAnimation();
            }
        });
    ...
    private void addCloseListener() {
        ...
        mAnimatorClose.addListener(new AnimatorListenerAdapter() {
            @Override
            public void onAnimationStart(final Animator animation) {
                if (null == mIAnimationView) {
                    return;
                }
                mIAnimationView.closeAnimation();
            }
    ...
    public void smoothScrollToPosition(final RecyclerView recyclerView, final RecyclerView.State state, final int position) {
        if (position >= getItemCount()) {
            Log.e(OldExpandLayoutManager.class.getName(), "Cannot scroll to " + position + ", item count is " + getItemCount());
            return;
        }
        ...
        final LinearSmoothScroller scroller = new LinearSmoothScroller(recyclerView.getContext()) {
            ...
            public PointF computeScrollVectorForPosition(final int targetPosition) {
                return OldExpandLayoutManager.this.computeScrollVectorForPosition(targetPosition);
            }
