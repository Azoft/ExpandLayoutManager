...
import com.azoft.layoutmanager.R;
import com.azoft.layoutmanager.utils.GeneralUtils;
...
public final class SimpleAnimationView extends FrameLayout implements AnimationView {
    ...
    public boolean doCollapseAnimation(final float animationProgress) {
        ...
        if (0.999999f < animationProgress) {
            hasChange = View.GONE != viewToAnimate.getVisibility();

            viewToAnimate.setVisibility(View.GONE);
            viewToAnimate.setRotationX(EXPAND_ANGLE);
        } else {
            hasChange = View.VISIBLE != viewToAnimate.getVisibility();
